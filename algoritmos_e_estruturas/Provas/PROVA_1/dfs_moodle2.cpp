...
int main() {
    ...
    if (orientado == 1) {
        ...
        while (cin >> origem >> destino && origem != -1 && destino != -1) {
            grafo[origem].push_back({origem, destino}); // Aresta orientada
        }
    } else {
        // Grafo não orientado
        while (cin >> origem >> destino && origem != -1 && destino != -1) {
            grafo[origem].push_back({origem, destino}); // Aresta não orientada
            grafo[destino].push_back({destino, origem}); // Adiciona a aresta reversa
        }
    }
